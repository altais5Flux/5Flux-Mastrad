...
namespace WebservicesSage
{
    public partial class MappingOrdersStaut : Form
    {
        ...
        private void SaveButton_Click(object sender, EventArgs e)
        {
            ...
            valuePresta = (SingletonUI.Instance.PrestaId1.selectedIndex + 1).ToString(); /*+ "_" + (SingletonUI.Instance.PrestaId2.selectedIndex + 1) + "_" + (SingletonUI.Instance.PrestaId3.selectedIndex + 1)*/
            if (Utils.UtilsConfig.MagentoStatutId.TryGetValue("default", out SavedStatut))
            {
                if (!valuePresta.Equals(SavedStatut))
                {
                    Utils.UtilsConfig.UpdateNodeInCustomSection("MagentoStatutId", "default", valuePresta);
                }
            }
            valueOrder = (SingletonUI.Instance.SageDoc1.selectedIndex + 1).ToString(); /*+ "_" + (SingletonUI.Instance.SageDoc2.selectedIndex + 1) + "_" + (SingletonUI.Instance.SageDoc3.selectedIndex + 1)*/
